...
- `house/batcontrol/decision` - Current mode with its value and the reason as text, e.g. `Charge from Grid 1250 W - Grid recharge required` or `Discharge Allowed - Usable energy exceeds reserve` (retained). Updated on a status change only: a new mode, or a change of the charge rate / PV limit of the current mode by 25 % or more, see [Decision Trace](../development/decision-trace.md#journal-and-status-change-listeners). Shown as the **Decision** sensor in Home Assistant
