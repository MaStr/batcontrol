...
from batcontrol.logic.logic_interface import CalculationInput
...
def test_config_parses_strategy_and_pv_factor():
    ...
    assert config.pv_forecast_factor == 0.5


def test_effective_min_grid_charge_soc_resolver_uses_calculation_input():
    config = GridChargeTargetConfig(strategy='forecast', pv_forecast_factor=0.5)
    calc_input = CalculationInput(
        production=np.array([149, 569, 1488, 2678, 3500, 4000]),
        consumption=np.array([547, 731, 3427, 3497, 3700, 500]),
        prices=np.array([0.4635, 0.7018, 0.7018, 0.7018, 0.7018, 0.4635]),
        stored_energy=870.4,
        stored_usable_energy=0.0,
        free_capacity=8243.2,
    )

    target = calculate_effective_min_grid_charge_soc(
        config=config,
    ...
    assert target == pytest.approx(0.79, abs=0.01)
